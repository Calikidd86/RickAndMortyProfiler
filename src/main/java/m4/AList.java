...
*/
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof AList<?>) {
            ...
            AList<T> copyList = new AList<T>(otherListParam.numberOfEntries);

            if (this.numberOfEntries != otherListParam.numberOfEntries) {
                return false;
            }

            for (int i = 1; i <= numberOfEntries; i++) {
                copyList.add(otherListParam.list[i]);
            }

            for (int i = 1; i < copyList.numberOfEntries; i++) {
                if (!this.list[i].equals(copyList.list[i])) {
                    return false;
                }
            }
            return true;
    ...
    public int compareTo(AList<T> o) {
        if (this.numberOfEntries < o.numberOfEntries) {
            return -11;
        } else if (this.numberOfEntries > o.numberOfEntries) {
            return 11;
        } else {
            for (int i = 1; i < this.numberOfEntries; i++) {
                if (!this.list[i].equals(o.list[i])) {
                    int comparisonResult = this.list[i].compareTo(o.list[i]);
